...
public class And_Login_Test_CaseGradePreKEnglish extends Hooks {
	...
	@Test
	public void AndroidLoginTestCaseGradePreKEnglish() throws InterruptedException, IOException {

		Device device = DeviceFactory.getInstance().getDevice("android", 8219);
		...
		PDS.ElaToggleIsTapped();
		Thread.sleep(2000);
		String exp1stEnglishCardName = "Minecart: I";
		String expSecondPlayable = "Sound Pops: I";
		String expPopupHeading = "Activity is locked";
		String expPopupSubHeading = "Please complete previous activities first.";
		Extent_Test_Manager.AssertEquals("Correct First card is getting displayed for Grade Prek English",
				PDS.ElementGetText(driver, "Minecart: I"), exp1stEnglishCardName);
		Extent_Test_Manager.AssertEquals(
				"English Dashboard 2nd correct playable is getting displayed for Grade Prek English ",
				PDS.ElementGetText(driver, "Sound Pops: I"), expSecondPlayable);
		...
		Thread.sleep(3000);
		String ExpFirstCard = "Can You Find A";
		Extent_Test_Manager.AssertEquals("Correct ELA Bottom drawer screen - Letters A - Z is getting displayed",
				PDS.ElementGetText(driver, "Can You Find A"), ExpFirstCard);
		PSC.ScreenshotTaken("ELA Bottom Drawer Screen - Letters A - Z ", Result);
		String expfirstmilestone = "Uppercase Letters A-Z";
		Extent_Test_Manager.AssertEquals("Correct 1st Milestone Cloud is getting displayed",
				PDS.ElementGetText(driver, "Uppercase Letters A-Z"), expfirstmilestone);

		PDS.scrollToTextAndClick(driver, "Can You Find A");
		Thread.sleep(22000);
		...
		try {
			...
			if (MGLS.alertIsDisplayed()) {
				...
				Extent_Test_Manager.AssertFail("Content Group screen - Letter A-Z first playable Display",
						"Content Group screen - Letter A-Z first playable Display");
			...
			PSC.ScreenshotTaken("Content Group screen - Letter A-Z first playable Display", Result);
		...
		Thread.sleep(3000);
		String ExpFirstTextSW = "We can read much faster when we can read words by sight.";
		Extent_Test_Manager.AssertEquals("Correct ELA Bottom drawer screen - Sight Words is getting displayed",
				PDS.ElementGetText(driver, "We can read much faster when we can read words by sight."), ExpFirstTextSW);
		PSC.ScreenshotTaken("Content Group Screen - Sight Words", Result);

		PDS.scrollToTextAndClick(driver, "I");
		Thread.sleep(20000);
		try {
		...
		PDS.scrollToTextAndClick(driver, "Leveled Readers");
		Thread.sleep(3000);
		String ExpText = "Read Leveled Books";
		Extent_Test_Manager.AssertEquals("Correct ELA Bottom drawer screen - Leveled Readers is getting displayed",
				PDS.ElementGetText(driver, "Read Leveled Books"), ExpText);
		PSC.ScreenshotTaken("Content Group Screen - Leveled Readers", Result);
		CG.LevelReaderFirstPlayableIsTapped();
		Thread.sleep(22000);
		try {
